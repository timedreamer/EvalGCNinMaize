...
# for maize, disgard all fifteen, but use fifteen_ranked instead 
...
p_fif_rank <- GO_agg_rankPccMaize[[1]][,1]
p_pr <- GO_pr_pcc[[1]][,1]
...
p_fif_rank <- GO_agg_rankMrnetMaize[[1]][,1]
...
p_fif_rank <- GO_agg_rankClrMaize[[1]][,1]
p_pr <- GO_pr_clr[[1]][,1]
...
# one-factor boxplot. export as svg 800*500, dashline is 15 experiment mean.
...
abline(h=0.54540656,lty=2)

boxplot(total_mrnet~total_name_mrnet,main="GO_MRNET",las=2,ylim=c(0.3,1),
        outcol="grey",whiskcol="grey",staplecol="grey",cex.axis=2,
        names=c("12","36","65","108","270","404","1266","six","fif_rank","protein"));box(lwd=2)
points(mean_mrnet,col="black",pch=8)
abline(h=0.590887133,lty=2)


boxplot(total_clr~total_name_clr,main="GO_CLR",las=2,ylim=c(0.3,1),
        outcol="grey",whiskcol="grey",staplecol="grey",cex.axis=2,
        names=c("12","36","65","108","270","404","1266","six","fif_rank","protein"));box(lwd=2)
points(mean_clr,col="black",pch=8)
abline(h=0.60344548,lty=2)
...
total_netname <- rep(c(rep("12",277),rep("36",277),rep("65",277),rep("108",277),rep("270",277),
                   rep("404",277),rep("1266",277),rep("six",277),
                   rep("fif_rank",277),rep("protein",307)),3)
