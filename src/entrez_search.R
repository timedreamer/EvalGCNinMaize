...
year <- 2008:2015
sra_maize <- sapply(year, search_year, term="(Zea mays[ORGN] OR maize[ORGN]) AND RNA-Seq[STRA] AND illumina[PLAT]",
                 USE.NAMES=FALSE)
...
plot(sra_maize,type="b",col="red",xaxt="n", main="Maize RNA-Seq vs Microarray Samples",
     ylab="number of samples",xlab="year")
lines(geo_maize,type="b",col="blue")
axis(1, at=1:8, labels=c(2008:2015))
