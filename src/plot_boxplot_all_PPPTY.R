...
total_name_pcc <- c(names(p_12),names(p_36),names(p_65),names(p_108),names(p_270),
                    names(p_404),names(p_1266),names(p_six),names(p_fif),names(p_fif_rank),names(p_pr))
total_name_pcc <- factor(total_name_pcc,level = c("12","36","65","108","270","404","1266",
                                                  "six","fifteen","fif_rank","protein"))
method_pcc <- rep("pcc",19612) # 1721*10 + 2402
...
boxplot(total_mrnet~total_name_mrnet,main="GO_MRNET",las=2,ylim=c(0.4,0.75),
        outcol="grey",whiskcol="grey",staplecol="grey",cex.axis=2,
        names=c("12","36","65","108","270","404","1266","six","fifteen","fif_rank","protein"));box(lwd=2)
points(mean_mrnet,col="black",pch=8)


boxplot(total_clr~total_name_clr,main="GO_CLR",las=2,ylim=c(0.4,0.75),
        outcol="grey",whiskcol="grey",staplecol="grey",cex.axis=2,
        names=c("12","36","65","108","270","404","1266","six","fifteen","fif_rank","protein"));box(lwd=2)
...
total_netname <- rep(c(rep("12",1721),rep("36",1721),rep("65",1721),rep("108",1721),rep("270",1721),
                       ...
                       rep("fifteen",1721),rep("fif_rank",1721),rep("protein",2402)),3)
