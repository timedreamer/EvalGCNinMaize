...
raw_data <- read.delim("XUbuntu_0811_titleProcessed.FC.txt", stringsAsFactors = FALSE,check.names = FALSE)
...
#mp <- barplot(y$samples$lib.size*1e-6,ylab="Library size(millions)") # bargraph show library size
#axis(1,at=mp,labels = sample_name,las=2,cex.axis=0.7)
abline(h=5,col="red") # this is lib size after remove a lot genes.
# libSize <- y$samples
# libSize[,libSize$lib.size<5000000]
# small <- subset(libSize,lib.size<5000000)

# Get gene length of filtered genes that can be used to count RPKM.
gene_name <- as.vector(y$genes$genes)
id <- as.vector(unname(raw_data$Geneid))
geneLength <- raw_data[id %in% gene_name,2]
summary(geneLength)

# Calculate cpm and rpkm
#counts.p.m <- cpm(y,normalized.lib.sizes = TRUE)
#log2 transformed
...
head(counts.p.m)[1:5,1:5]

y$genes$Length <- geneLength
#reads.p.m <- rpkm(y,normalized.lib.sizes = TRUE)
#log2 transformed
...
avg_expr1 <- apply(X = unname(reads.p.m[1:1266]),MARGIN = 1,FUN = mean)
head(avg_expr1)
plot(reads.p.m$geneLength,avg_expr1,main="RPKM_avgExpr vs GeneLength")
plot(density(avg_expr1),main="RPKM_density_avgExpr")
summary(avg_expr1)
##only choose gene length from 100- 3000.
exp_f1 <- reads.p.m[which(reads.p.m$geneLength>100),]
exp_f1 <- exp_f1[which(exp_f1$geneLength<3000),]
colnames(exp_f1)
nrow(exp_f1)
avg_expr2 <- apply(X = unname(exp_f1[1:1266]),MARGIN = 1,FUN = mean)
length(avg_expr2)
plot(exp_f1$geneLength,avg_expr2,main="RPKM_avgExpr_100_3000")
...
avg_expr3 <- apply(X = unname(counts.p.m[1:1266]),MARGIN = 1,FUN = mean)
head(avg_expr3)
plot(counts.p.m$geneLength,avg_expr3,main="CPM_avgExpr vs GeneLength")
plot(density(avg_expr3),main="CPM_density_avgExpr")
summary(avg_expr3)
##only choose gene length from 100- 3000.
exp_f2 <- counts.p.m[which(counts.p.m$geneLength>100),]
exp_f2 <- exp_f2[which(exp_f2$geneLength<3000),]
colnames(exp_f2)
nrow(exp_f2)
avg_expr4 <- apply(X = unname(exp_f2[1:1266]),MARGIN = 1,FUN = mean)
length(avg_expr4)
plot(exp_f2$geneLength,avg_expr4,main="CPM_avgExpr_100_3000")
...
# clean. Keep gene_name
rm(raw_data,raw_data1,geneLength,id,keep,y)
rm(exp_f1,exp_f2,avg_exp,avg_expr1,avg_expr2,avg_expr3,avg_expr4,avg_exp_high)
...
#plot(density(as.matrix(filter_data)),main="RawCount_allGeneExprDistribution") # raw_count density
max(filter_data)
#avg_expr5 <- apply(X = unname(filter_data[1:115]),MARGIN = 1,FUN = mean) # raw_count avgExpr
avg_expr5 <- apply(X = unname(log2(filter_data[1:1266])),MARGIN = 1,FUN = mean)
plot(geneLength,avg_expr5,main="rawCount_avgExpr vs GeneLength") # gene Length and avgExpr rawCount
plot(density(avg_expr5),main="rawCount_density_avgExpr") # density avgExpr of rawCount

colnames(filter_data)
raw_log <- log2(filter_data+1)
head(raw_log)[1:5,1:5]
write.table(x = raw_log,file = "rawCount_log2_result.txt",sep = "\t",quote = FALSE)

#save colnames which are file naems into seperate files named "colData".
...
par(mfrow=c(2,2))
vst_expr <- assay(vst)
vst_expr <- cbind(vst_expr,geneLength) # last column is the geneLength
head(vst_expr)
colnames(vst_expr);ncol(vst_expr)
plot(density(vst_expr[,1:1266]),main="VST_allGeneExprDistribution")
max(vst_expr[,1:1266])
avg_expr6 <- apply(X = unname(vst_expr[,1:1266]),MARGIN = 1,FUN = mean) # VST avgExpr
plot(vst_expr[,1266],avg_expr6,main="VST_avgExpr vs GeneLength",xlab="geneLength") # gene Length and avgExpr rawCount
plot(density(avg_expr6),main="VST_density_avgExpr") # density avgExpr of VST

##only choose gene length from 100- 3000.
exp_f3 <- vst_expr[which(vst_expr[,1267]>100),]  # col 1267 is the geneLength
exp_f3 <- exp_f3[which(exp_f3[,1267]<3000),]
colnames(exp_f3)
nrow(exp_f3)
avg_expr7 <- apply(X = unname(exp_f3[,1:1266]),MARGIN = 1,FUN = mean)
length(avg_expr7)
plot(exp_f3[,1267],avg_expr7,main="VST_avgExpr_100_3000",xlab="geneLength")

#clean
rm(dds,vst,cdInput,filter_data,result_vst,sampleTree,sampleDistMatrix)
rm(d,exp_f3,vst_expr)
rm(avg_expr5,avg_expr6,avg_expr7,gene_name)
