#for arabidopsis, disgard all, use all_rank
...
#PCC
p_1363 <- GO_arab_pcc[[1]][,1];p_allRank <- GO_arab_aggRank_pcc[[1]][,1]
total_pcc <- c(p_1363,p_allRank)
total_name_pcc <- c(rep("1363",445),rep("all_rank",445))
total_name_pcc <- factor(total_name_pcc,level = c("1363","all_rank"))
method_pcc <- rep("pcc",890) #
mean_pcc <- c(mean(p_1363),mean(p_allRank))

#mrnet
p_1363 <- GO_arab_mrnet[[1]][,1];p_allRank <- GO_arab_aggRank_mrnet[[1]][,1]
total_mrnet <- c(p_1363,p_allRank)
total_name_mrnet <- c(rep("1363",445),rep("all_rank",445))
total_name_mrnet <- factor(total_name_mrnet,level = c("1363","all_rank"))
method_mrnet <- rep("mrnet",890) #
mean_mrnet <- c(mean(p_1363),mean(p_allRank))

#clr
p_1363 <- GO_arab_clr[[1]][,1];p_allRank <- GO_arab_aggRank_clr[[1]][,1]
total_clr <- c(p_1363,p_allRank)
total_name_clr <- c(rep("1363",445),rep("all_rank",445))
total_name_clr <- factor(total_name_clr,level = c("1363","all_rank"))
method_clr <- rep("clr",890) #
mean_clr <- c(mean(p_1363),mean(p_allRank))



# one-factor boxplot. export svg 500*500. 
# dash lines are mean value for 44 experiment
...
abline(h=0.561748591,col="black",lty=2)
...
abline(h=0.591206807,col="black",lty=2)
...
mean_value_all <- c(0.6966909,0.7591181,0.7825401,0.7679227,0.7591242,0.7630428)
# 
...
boxplot(total_value~total_constructMethod*total_netname,las=2,yaxt="n",
        col=c("white","grey","grey34"),ylim=c(0.4,1),cex.axis=1.5,
        outcol="grey",whiskcol="grey",staplecol="grey",main="allNtwk")
box(lwd=2);axis(2,cex.axis=1.5,las=1)
