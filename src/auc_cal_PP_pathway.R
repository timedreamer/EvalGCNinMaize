...
auc_vst_gcc <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=vst_gcc_ft);rm(vst_gcc,vst_gcc_ft,node_name)         #
                                                                                                                         #
...
auc_vst_bicor <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=vst_bicor_ft);rm(vst_bicor,vst_bicor_ft)           #
                                                                                                                         #
vst_aaracne_ft <- vst_aaracne[,which(colnames(vst_aaracne) %in% filter_node_name)]                                       #
auc_vst_aaracne <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=vst_aaracne_ft);rm(vst_aaracne,vst_aaracne_ft)   #
                                                                                                                         #
vst_maracne_ft <- vst_maracne[,which(colnames(vst_maracne) %in% filter_node_name)]                                       #
auc_vst_maracne <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=vst_maracne_ft);rm(vst_maracne,vst_maracne_ft)   #
                                                                                                                         #
...
auc_vst_clr <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=vst_clr_ft);rm(vst_clr,vst_clr_ft)                   #
                                                                                                                         #
method <- c("gcc","pcc","scc","kcc","bicor","aaracne","marance","mrnet","clr")                                           #
...
auc_cpm_gcc <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=cpm_gcc_ft);rm(cpm_gcc,cpm_gcc_ft,node_name)         #
                                                                                                                         #
...
auc_cpm_bicor <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=cpm_bicor_ft);rm(cpm_bicor,cpm_bicor_ft)           #
                                                                                                                         #
cpm_aaracne_ft <- cpm_aaracne[,which(colnames(cpm_aaracne) %in% filter_node_name)]                                       #
auc_cpm_aaracne <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=cpm_aaracne_ft);rm(cpm_aaracne,cpm_aaracne_ft)   #
                                                                                                                         #
cpm_maracne_ft <- cpm_maracne[,which(colnames(cpm_maracne) %in% filter_node_name)]                                       #
auc_cpm_maracne <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=cpm_maracne_ft);rm(cpm_maracne,cpm_maracne_ft)   #
                                                                                                                         #
...
auc_cpm_clr <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=cpm_clr_ft);rm(cpm_clr,cpm_clr_ft)                   #
                                                                                                                         #
method <- c("gcc","pcc","scc","kcc","bicor","aaracne","marance","mrnet","clr")                                           #
save(paste0("auc_cpm_",method),file="AUC_cpm.RData")                                                                     #
##########################################################################################################################
...
auc_rc_gcc <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=rc_gcc_ft);rm(rc_gcc,rc_gcc_ft,node_name)             #
                                                                                                                         #
...
auc_rc_bicor <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=rc_bicor_ft);rm(rc_bicor,rc_bicor_ft)               #
                                                                                                                         #
rc_aaracne_ft <- rc_aaracne[,which(colnames(rc_aaracne) %in% filter_node_name)]                                          #
auc_rc_aaracne <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=rc_aaracne_ft);rm(rc_aaracne,rc_aaracne_ft)       #
                                                                                                                         #
rc_maracne_ft <- rc_maracne[,which(colnames(rc_maracne) %in% filter_node_name)]                                          #
auc_rc_maracne <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=rc_maracne_ft);rm(rc_maracne,rc_maracne_ft)       #
                                                                                                                         #
...
auc_rc_clr <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=rc_clr_ft);rm(rc_clr,rc_clr_ft)                       #
                                                                                                                         #
method <- c("gcc","pcc","scc","kcc","bicor","aaracne","marance","mrnet","clr")                                           #
save(paste0("auc_rc_",method),file="AUC_rc.RData")                                                                       #
##########################################################################################################################
...
auc_rpkm_gcc <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=rpkm_gcc_ft);rm(rpkm_gcc,rpkm_gcc_ft,node_name)         #
                                                                                                                             #
...
auc_rpkm_bicor <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=rpkm_bicor_ft);rm(rpkm_bicor,rpkm_bicor_ft)           #
                                                                                                                             #
rpkm_aaracne_ft <- rpkm_aaracne[,which(colnames(rpkm_aaracne) %in% filter_node_name)]                                        #
auc_rpkm_aaracne <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=rpkm_aaracne_ft);rm(rpkm_aaracne,rpkm_aaracne_ft)   #
                                                                                                                             #
rpkm_maracne_ft <- rpkm_maracne[,which(colnames(rpkm_maracne) %in% filter_node_name)]                                        #
auc_rpkm_maracne <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=rpkm_maracne_ft);rm(rpkm_maracne,rpkm_maracne_ft)   #
                                                                                                                             #
...
auc_rpkm_clr <- sapply(X = filter_node_name,FUN = calc_auc,corMatrix=rpkm_clr_ft);rm(rpkm_clr,rpkm_clr_ft)                   #
                                                                                                                             #
method <- c("gcc","pcc","scc","kcc","bicor","aaracne","marance","mrnet","clr")                                               #
save(paste0("auc_rpkm_",method),file="AUC_rpkm.RData")                                                                       #
##############################################################################################################################
...
auc_rpkm_all_median <- c(median(auc_rpkm_pcc),median(auc_rpkm_scc),median(auc_rpkm_gcc),median(auc_rpkm_kcc),median(auc_rpkm_bicor),
                       median(auc_rpkm_aaracne),median(auc_rpkm_maracne),median(auc_rpkm_mrnet),median(auc_rpkm_clr))
